...
def main(in_lca, in_bam, out_lca, out_bam, out_stats, out_subs, stranded, mincount, k, upto, minsim, exclude_keywords):
    ...
    # Adds a "\t" before and a ":" after strings with only digits (assume tax ids), and adds a ":" and a ":" after only characters (assume tax names)
    # This just makes for a safer check later, so you don't catch tax id 4390 when you specified tax id 439 for example
    formatted_keywords = []
...
if __name__ == "__main__":
    ...
    if args.exclude_keyword_file:
        ...
        with open(args.exclude_keyword_file, 'r') as f:
            args.exclude_keywords = [line.strip() for line in f if line.strip()]
